...
Fixpoint count (v : nat) (s : bag) : nat :=
  ...
  | [] => 0
  | h :: t =>
      if Nat.eqb h v then S (count v t)
      else count v t
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  ...
  | [] => []
  | h :: t =>
      if Nat.eqb h v then remove_all v t
...
Proof.
  intros l1 l2.               (* Introduce the lists into the context *)
  induction l1 as [| h t IH].  (* Induction on l1 *)
  - (* Base case: l1 = [] *)
    simpl.                     (* rev ([] ++ l2) = rev l2 *)
    rewrite app_nil_r.         (* rev l2 ++ [] = rev l2 *)
    reflexivity.
  - (* Inductive step: l1 = h :: t *)
    simpl.                     (* rev ((h :: t) ++ l2) = rev (t ++ l2) ++ [h] *)
    rewrite IH.                (* Apply the induction hypothesis *)
    rewrite app_assoc.         (* Rearrange using associativity of ++ *)
    reflexivity.
Qed.
...
Proof.
  intros l1 l2.                (* Introduce the lists *)
  induction l1 as [| h t IH].  (* Induction on l1 *)
  - (* Base case: l1 = [] *)
    simpl.                     (* nonzeros ([] ++ l2) = nonzeros l2 *)
    reflexivity.               (* [] ++ nonzeros l2 = nonzeros l2 *)
  - (* Inductive step: l1 = h :: t *)
    simpl.                     (* nonzeros ((h::t) ++ l2) = ... *)
    destruct h as [| n].       (* Check if h = 0 or h = S n *)
    + (* Case h = 0 *)
      apply IH.                (* nonzeros (0 :: t ++ l2) = nonzeros t ++ nonzeros l2 *)
    + (* Case h = S n *)
      simpl. rewrite IH.       (* nonzeros ((S n)::t ++ l2) = (S n) :: (nonzeros t ++ nonzeros l2) *)
      reflexivity.
Qed.
(** [] *)
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | [], [] => true
  | h1 :: t1, h2 :: t2 =>
      if Nat.eqb h1 h2 then eqblist t1 t2 else false
  | _, _ => false
  end.
...
Proof.
  intros x.
  destruct x as [n].       (* x = Id n *)
  induction n as [| n' IH].
  - (* n = 0 *)
    simpl. reflexivity.
  - (* n = S n' *)
    simpl.                  (* (S n') =? (S n') reduces to n' =? n' recursively *)
    apply IH.
Qed.
(** [] *)
...
Proof.
  intros d x v.
  (* unfold update to see record *)
  simpl.                       (* update d x v = record x v d *)
  (* find x (record x v d) *)
  rewrite eqb_id_refl.          (* eqb_id x x = true *)
  reflexivity.
Qed.
(** [] *)
...
Proof.
  intros d x y o H.
  simpl.                 (* update d y o = record y o d *)
  (* find x (record y o d) = if eqb_id x y then Some o else find x d *)
  rewrite H.             (* eqb_id x y = false *)
  reflexivity.
Qed.
(** [] *)
End PartialMap.
