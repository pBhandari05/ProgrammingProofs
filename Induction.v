...
(** For this [Require] command to work, Rocq needs to be able to
    ...
    the current directory. I recommend making separate short (in class)
    and full (homework) folders, each containing all of the files from
    the corresponding column on Canvas.
    ...
    line.  From an IDE...
     ...
     - For VSCode, open the terminal pane at the bottom and then
    ...
    To compile [Basics.v] from the command line...
     ...
     - First, generate a [Makefile] using the [rocq makefile] utility,
       which comes installed with Rocq.
...
(** **** Exercise: 2 stars, advanced, optional (add_comm_informal)
    ...
    Proof: (* FILL IN HERE *)
   - Suppose [n = 0]. We try to show that 0 + m = m + 0. This follows directly from the definition of [+].

    - Now we suppose [n = S n'], where n' + m = m + n'.

      We need to see (S n') + m = m + (S n').

      By def of [+], this follows from S (n' + m) = m + S n'.

      Rewrite using plus_n_Sm, this follows from S (n' + m) = S (m + n').

      which is clear by induction hypothesis. Boom.
*)
...
(* ################################################################# *)
(** * Nat to Bin and Back to Nat (Advanced) *)
...
(** **** Exercise: 3 stars, advanced (binary_commute)
...
Proof.
  ...
  - simpl. rewrite add_0_r. rewrite add_0_r. rewrite IHb. simpl. rewrite add_comm. simpl. reflexivity.
Qed.
(** [] *)

(** **** Exercise: 3 stars, advanced (nat_bin_nat) *)
...
(* The theorem fails because binary numbers in our representation are not unique.
Different values of type [bin] can represent the same natural number.

For example, [Z], [B0 Z], [B0 (B0 Z)], etc., all represent the number 0.
Similarly, a value like [B0 b] represents the same number as shifting [b],
but is not necessarily the canonical (simplest) form.

The function [bin_to_nat] forgets the binary structure and keeps only the
numeric value. When we convert back using [nat_to_bin], we get a canonical
binary representation, not necessarily the original [bin].

Thus, [nat_to_bin (bin_to_nat b)] produces a binary number equivalent to [b],
but not definitionally equal to [b], causing the theorem to fail.*)
...
Lemma double_nat_bin : forall n, nat_to_bin (n + n) = double_bin (nat_to_bin n).
Proof.
  ...
  - (* n = S n' *)
    simpl. 
    rewrite <- plus_n_Sm. (* Move S out: S n' + S n' = S (S (n' + n')) *)
    simpl. 
    rewrite IHn'. 
    (* We need to show: incr (incr (double_bin ...)) = double_bin (incr ...) *)
...
induction b as [|b' IHb'|b' IHb'].
  ...
  - (* b = B0 b' *)
    simpl.
    (* bin_to_nat (B0 b') is (2 * val b'), which is val b' + val b' *)
    rewrite add_0_r. 
  ...
  - (* b = B1 b' *)
    simpl.
    (* bin_to_nat (B1 b') is 1 + 2 * val b' *)
    rewrite add_0_r.
    (* We have nat_to_bin (1 + (val + val)). Pull out the 1 (incr). *)
    simpl.
    rewrite double_nat_bin.
    rewrite IHb'.
