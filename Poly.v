...
- Base case: l = [].
  Then length [] = 0, so the hypothesis length l = n gives n = 0.
...
- Inductive case: l = a :: l'.
  Assume as the induction hypothesis:
    IH: length l' = n' -> nth_error l' n' = None
    for all n'.
  We need to show:
    length (a :: l') = n -> nth_error (a :: l') n = None.

  Suppose length (a :: l') = n.
  By simplification of length, this means:
    S (length l') = n.

  Now consider nth_error (a :: l') n:
    By definition of nth_error:
      nth_error (a :: l') n =
        if n =? 0 then Some a else nth_error l' (pred n).

  But since n = S (length l'), we know n ≠ 0.
  Therefore the if-condition fails, and we are left with:
      nth_error (a :: l') n = nth_error l' (pred n).

  Using n = S (length l'), we compute pred n:
      pred (S (length l')) = length l'.

  So we reduce to:
      nth_error (a :: l') n = nth_error l' (length l').

  Now apply the induction hypothesis IH with n' := length l':
      IH: length l' = length l' -> nth_error l' (length l') = None.

  The premise is true by reflexivity, so:
      nth_error l' (length l') = None.

  Therefore:
      nth_error (a :: l') n = None.

  This completes the inductive step.

Thus, by induction on l, we have shown:
  forall X l n, length l = n -> nth_error l n = None.
Qed.
